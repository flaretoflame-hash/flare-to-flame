...
Camera move per beat: Vulnerability = Handheld Drift (never shaky) | Transformation = Slow Push In / Orbit / Slow reveal | Detail = Slider / micro Push In with a subject action (moves; static no longer allowed, Buddy 19 Sep 2026) | Atmosphere = Lateral Slide | Invitation = Slow Pull Back / Static hold.
Negative: official Wan negative (includes static-frame terms) + the FTF negatives.
Runner: ftf-coworkers/conductor/conductor-runs/angle-016/workflows/broll_motionfix.py (Beats 2, 3, 5 by default; Beat 4 Detail with --beats 4, optionally --start-image FILE for prop conditioning; profiles standard / extended; outputs tagged _s<shift>_f<frames>). Every clip gets a motion score and a jerk number. Problems: scripts/ftf_diag.py and handoff/ftf-problem-kit.md (RULE 29). The runner stops before generating if any node, input or model name differs from the installed ComfyUI, then prints old-vs-new motion scores.
Beat 5 (decided 19 Sep 2026): room lateral slide = baseline and universal fallback. A service-specific atmosphere (tool, prop, texture, preparation action, calm hand movement) is allowed only from the service's approved Atmosphere Elements (Airtable field planned, not yet created, so every service uses the room slide today). Never invented. Runner keeps the room slide.
